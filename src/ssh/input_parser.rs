...
pub(super) fn parse_event(buffer: &[u8], input_available: bool) -> Result<Option<KeyEvent>> {
    match parse_event_internal(buffer, input_available) {
        Ok(event) => {
            if let Some(event) = event {
                match event {
                    InternalEvent::Event(Event::Key(key_event)) => Ok(Some(key_event)),
...
fn parse_event_internal(buffer: &[u8], input_available: bool) -> Result<Option<InternalEvent>> {
    if buffer.is_empty() {
        return Ok(None);
    }
        ...
        b'\x1B' => {
            if buffer.len() == 1 {
                ...
                match buffer[1] {
                    ...
                    _ => parse_event_internal(&buffer[1..], input_available).map(|event_option| {
...
mod tests {
    ...
    fn test_esc_key() {
        assert_eq!(
            parse_event_internal(b"\x1B", false).unwrap(),
    ...
    fn test_possible_esc_sequence() {
        assert_eq!(parse_event_internal(b"\x1B", true).unwrap(), None,);
    }

    #[test]
    fn test_alt_key() {
        assert_eq!(
            parse_event_internal(b"\x1Bc", false).unwrap(),
    ...
    fn test_alt_shift() {
        assert_eq!(
            parse_event_internal(b"\x1BH", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('H'),
    ...
    fn test_alt_ctrl() {
        assert_eq!(
            parse_event_internal(b"\x1B\x14", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('t'),
    ...
    fn test_parse_event_subsequent_calls() {
        ...
        assert_eq!(
            parse_event_internal(b"\x1B[20;10R", false).unwrap(),
            Some(InternalEvent::CursorPosition(9, 19))
        );

        // parse_csi
        assert_eq!(
            parse_event_internal(b"\x1B[D", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyCode::Left.into()))),
        );

        // parse_csi_modifier_key_code
        assert_eq!(
            parse_event_internal(b"\x1B[2D", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Left,
                KeyModifiers::SHIFT
            ))))
        ...
        assert_eq!(
            parse_event_internal(b"\x1B[3~", false).unwrap(),
        ...
        #[cfg(feature = "bracketed-paste")]
        assert_eq!(
            parse_event_internal(b"\x1B[200~on and on and on\x1B[201~", false).unwrap(),
        ...
        assert_eq!(
            parse_event_internal(b"\x1B[32;30;40;M", false).unwrap(),
        ...
        assert_eq!(
            parse_event_internal(b"\x1B[M0\x60\x70", false).unwrap(),
            Some(InternalEvent::Event(Event::Mouse(MouseEvent {
                kind: MouseEventKind::Down(MouseButton::Left),
                column: 63,
                row: 79,
        ...
        // parse_csi_sgr_mouse
        assert_eq!(
            parse_event_internal(b"\x1B[<0;20;10;M", false).unwrap(),
            Some(InternalEvent::Event(Event::Mouse(MouseEvent {
                kind: MouseEventKind::Down(MouseButton::Left),
                column: 19,
                row: 9,
        ...
        // parse_utf8_char
        assert_eq!(
            parse_event_internal("Ž".as_bytes(), false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('Ž'),
                KeyModifiers::SHIFT
            )))),
    ...
    #[test]
    fn test_parse_event_internal() {
        assert_eq!(
            parse_event_internal(b"\t", false).unwrap(),
    ...
    fn test_parse_csi_bracketed_paste() {
        //
        assert_eq!(
            parse_event_internal(b"\x1B[200~o", false).unwrap(),
            None,
            "A partial bracketed paste isn't parsed"
        );
        assert_eq!(
            parse_event_internal(b"\x1B[200~o\x1B[2D", false).unwrap(),
            None,
            "A partial bracketed paste containing another escape code isn't parsed"
        );
        assert_eq!(
            parse_event_internal(b"\x1B[200~o\x1B[2D\x1B[201~", false).unwrap(),
    ...
    fn test_parse_char_event_lowercase() {
        assert_eq!(
            parse_event_internal(b"c", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('c'),
                KeyModifiers::empty()
            )))),
    ...
    fn test_parse_csi_special_key_code_with_types() {
        assert_eq!(
            parse_event_internal(b"\x1B[;1:3B", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new_with_kind(
                KeyCode::Down,
                KeyModifiers::empty(),
                KeyEventKind::Release,
        ...
        assert_eq!(
            parse_event_internal(b"\x1B[1;1:3B", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new_with_kind(
                KeyCode::Down,
                KeyModifiers::empty(),
                KeyEventKind::Release,
